	...
	refresh: function (frm) {
		if (frm.doc.docstatus === 1) {
			if (
				...
				flt(frm.doc.per_delivered, precision("per_delivered", doc)) < 100 &&
				flt(frm.doc.per_billed, precision("per_billed", doc)) < 100 &&
				frm.has_perm("write")
			) {
		...
		if (doc.docstatus == 1) {
			if (this.frm.has_perm("submit")) {
				if (doc.status === "On Hold") {
					...
					if (
						flt(doc.per_delivered, precision("per_delivered", doc)) < 100 ||
						flt(doc.per_billed, precision("per_billed", doc)) < 100
					) {
						// close
			...
			if (doc.status !== "Closed") {
				if (doc.status !== "On Hold") {
					...
					if (
						...
						flt(doc.per_picked, precision("per_picked", doc)) < 100 &&
						flt(doc.per_delivered, precision("per_delivered", doc)) < 100 &&
